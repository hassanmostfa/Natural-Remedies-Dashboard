...
import { FaEye, FaTrash, FaUser, FaGraduationCap } from 'react-icons/fa6';
...
const Instructors = () => {
  ...
  // API hooks
  const { data: instructorsResponse, isLoading, isError, refetch } = useGetInstructorsQuery({
  ...
  const [updateInstructorStatus, { isLoading: isUpdatingStatus }] = useUpdateInstructorStatusMutation();

  // Extract data and pagination from response
  const instructorsData = instructorsResponse?.data || [];
  const pagination = instructorsResponse?.pagination || null;
  ...
  const columns = [
    columnHelper.accessor('image', {
      ...
      cell: (info) => (
        <Avatar
          ...
          size="md"
          borderRadius="lg"
          fallback={<Icon as={FaGraduationCap} color="gray.500" />}
        />
      ),
    }),
        ...
        <VStack align="start" spacing={1}>
          ...
          <Badge 
            colorScheme={
              ...
              info.row.original.status === 'inactive' ? 'orange' : 'red'
            }
            width="fit-content"
            fontSize="xs"
          >
    ...
    columnHelper.accessor('courses_count', {
      ...
      cell: (info) => (
        <Badge
          colorScheme={info.getValue() > 0 ? 'green' : 'gray'}
          px="3"
          py="1"
          borderRadius="full"
        >
          {info.getValue()} courses
        </Badge>
      ),
    }),
      ...
      cell: (info) => (
        <Flex align="center" gap={2}>
          <Icon
            ...
            as={FaTrash}
            cursor="pointer"
            onClick={() => handleDeleteInstructor(info.getValue())}
          ...
          <Icon
            ...
            as={EditIcon}
            cursor="pointer"
            onClick={() => navigate(`/admin/edit-instructor/${info.getValue()}`)}
          ...
          {/* <Icon
            ...
            as={FaEye}
            cursor="pointer"
            onClick={() => navigate(`/admin/instructor/${info.getValue()}`)}
  ...
  const handleDeleteInstructor = async (id) => {
    try {
      ...
      toast({
        title: 'Error',
        description: error.data?.message || 'Failed to delete instructor.',
        status: 'error',
        duration: 3000,
        isClosable: true,
      });
      ...
      <Box mt="80px">
        ...
        >
          <Flex justify="center" align="center" h="200px">
            <Text color={textColor}>Loading instructors...</Text>
          </Flex>
        </Card>
      </Box>
    );
  ...
  return (
    <Box mt="80px">
      ...
      >
        ...
        {/* No data message */}
        {(!instructorsData || instructorsData.length === 0) && !isLoading && (
          <Flex
            px={{ base: "16px", md: "25px" }}
            py="40px"
            justify="center"
          ...
          >
            <Text color={textColor} fontSize="md">
              No instructors found with the current search.
            </Text>
          </Flex>
        )}
      </Card>
